...
def tag_list(request):
    ...
    ).annotate(games_count=Count('games', distinct=True))  # 使用distinct避免重复计数
    
    # 记录初始查询SQL
    logger.info(f"初始查询: {str(tags_query.query)}")
    # 不需要额外的查询提示，因为我们已经在模型中定义了适当的索引
    # 记录初始查询状态
    logger.info(f"初始查询状态: 搜索={bool(search_query)}, 状态过滤={bool(status_filter)}")
    ...
    # 使用Django的切片操作在数据库层面进行分页
    # 这会转换为SQL的LIMIT和OFFSET子句
    paginated_query = tags_query[offset:offset + page_size]
    
    # 记录分页SQL查询
    logger.info(f"分页SQL: {str(paginated_query.query)}")
    
    # 执行查询并获取结果
    tags = list(paginated_query)  # 强制执行查询并转为列表
